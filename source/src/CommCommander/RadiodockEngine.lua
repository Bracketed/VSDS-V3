...
local RadioController = script.Parent.Parent:FindFirstChild('CommCommanderController') or nil
...
local fakeRadioStates = { [1] = true, [2] = true, [3] = true, [4] = true, [5] = true, [6] = true }
...
function isUserWhitelisted(player)
	...
	for _, UserId in pairs(settings.config.whitelist.users_whitelisted) do if (player.UserId == UserId) then return true end end
	return false
end
...
function eventuallyShowAgain(radioIndex)
	local RadioInstance = script.Parent.DockParts.FakeRadios:FindFirstChild('FakeRadio'..radioIndex) or nil
...
script.Parent.TouchPart.PromptHolder.ProximityPrompt.Triggered:Connect(function(plr)
...
API.Event:Connect(function(event, params)
	if event == 'SetRadioControllerState' then
		if not (params.State) then script.Parent.TouchPart.PromptHolder.ProximityPrompt.Enabled = false return end
		script.Parent.TouchPart.PromptHolder.ProximityPrompt.Enabled = true
		return
	end
end)
  end
