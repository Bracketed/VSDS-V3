...
if not (settings.config.power_controller) then script.Parent.PowerButton.ClickDetector.MaxActivationDistance = 0 end
...
script.Parent.PowerButton.ClickDetector.MouseClick:Connect(function(plr)
	...
	if (OnlineState) then
		...
		UI.Enabled = false
		task.wait(1)
		script.Parent.ControllerParts.LCD.Color = Color3.fromRGB(36, 36, 36)
