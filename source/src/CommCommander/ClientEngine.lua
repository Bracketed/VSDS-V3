return {
    run = function(script, ...)
        ...
        local UI = LocalPlayer.PlayerGui:WaitForChild(
                       'CommCommanderRadioUserInterface')
        local CommCommanderService = game:GetService('ReplicatedStorage')
                                         :WaitForChild(
                                             '##CommCommanderRadioSystem##')
        local CommCommander = CommCommanderService:WaitForChild(
                                  '_CommCommanderConnector').Value
        local settings = require(CommCommander.CommCommanderConfig)
        local uiConfig = require(CommCommander.CommCommanderConfig
        ...
        uiConfig.defaults.color_schemes.custom = settings.ui_config
                                                     .custom_color_scheme
        if (uiConfig.defaults.color_schemes[string.lower(settings.ui_config
                                                             .color_scheme)]) then
            theme = uiConfig.defaults.color_schemes[string.lower(
        ...
        function detectBackpackRadioTool()
            for _, Tool in pairs(LocalPlayer:WaitForChild('Backpack')
                                     :GetChildren()) do
        ...
        function play(sound) return sound end

        if not (isUserWhitelisted(LocalPlayer)) then
            UI:Destroy()
            print('CommCommander Radio Client has finished loading! Took ' ..
                      string.sub(tick() - Tick, 1, 5) ..
                      ' seconds to load. CommCommander has loaded on user: ' ..
                      LocalPlayer.Name)
        ...
        function MessageUI.isPingMessage(MsgContent)
            if (string.find(MsgContent, '@')) then
                ...
                for _, str in pairs(StringSplit) do
                    if (string.find(str, '@') and
                        (string.find(str, LocalPlayer.Name) or
                            string.find(str, LocalPlayer.DisplayName))) then
                        return true
                    end
                end
            end
        ...
        function MessageUI.sysMsg(Content)
            ...
            if #UI.RadioInterface['RowB-RadioChat']:GetChildren() > 7 then
                ...
                for _, Object in pairs(
                                     UI.RadioInterface['RowB-RadioChat']:GetChildren()) do
            ...
            uiConfig.ui.system_message(Color3.fromRGB(0, 0, 0),
                                       Color3.fromRGB(132, 132, 132), Content)
                .Parent = UI.RadioInterface['RowB-RadioChat']
            play('message')
        end

        function MessageUI.new(MessageTypeInstance, Content, Sender)
            MessageTypeInstance = string.lower(MessageTypeInstance)
            if (MessageUI.isPingMessage(Content)) then
                MessageTypeInstance = 'ping'
                Content = MessageUI.formatPingMessage(Content)
            end
            if not (UplinkActive) then
                MessageTypeInstance = 'offline_msg'
                Content = Content .. ' - [ MESSAGE NOT DELIVERED ]'
            end

            local MessageTypes = {
                'message', 'ping', 'priority_msg', 'offline_msg'
            }

            if not table.find(MessageTypes, MessageTypeInstance) then
                return
            end
            ...
            uiConfig.ui.message(BackgroundColor, FlagColor, Content, Sender)
                .Parent = UI.RadioInterface['RowB-RadioChat']
            if Sender.userId == LocalPlayer.UserId then
                play('send')
            else
                play(MessageTypeInstance)
            end
        end
        ...
        UI.RadioInterface['RowA-RadioInfo'].RadioInfoState.RadioStatus
            .TextColor3 = Color3.fromRGB(255, 69, 69)
        CommCommanderService:WaitForChild('_CommCommanderServer'):FireServer(
            'SetRadioDetails',
            {["radioName"] = radio_name, ['radioChannel'] = channel})
        UI.RadioInterface['RowA-RadioInfo'].RadioInfoState.RadioTitle.Text =
            string.format('Radio - @%s | Name: %s', LocalPlayer.Name, radio_name)
        ...
        LocalPlayer.Backpack.ChildRemoved:Connect(function(Tool)
            ...
            if (settings.config.require_tool) then
                if not Tool:FindFirstChild('CommCommanderRadioTool') then
                    return
                end
        ...
        CommCommanderService:WaitForChild('_CommCommanderClient').OnClientEvent:Connect(
            function(RadioEvent, EventDetails)
                    ...
                    if EventDetails['RadioChannel'] == channel then
                        if UplinkActive then
                            if not (EventDetails['RadioMessageType']) then
                                MessageUI.new('message',
                                              EventDetails['RadioMessage'],
                                              EventDetails['RadioUser'])
                            else
                                MessageUI.new(EventDetails['RadioMessageType'],
                                              EventDetails['RadioMessage'],
                                              EventDetails['RadioUser'])
                            end
                        else
                            if EventDetails['RadioUser'].userId ==
            ...
            if touch.UserInputType == Enum.UserInputType.Keyboard then
                if touch.KeyCode.Name == Keybind then
                    if RadioEnabled then
                        RadioEnabled = false
                        UI.RadioInterface['RowA-RadioInfo'].RadioInfoState
                            .RadioStatus.TextColor3 =
                            Color3.fromRGB(255, 69, 69)
                        UI.RadioInterface['RowA-RadioInfo']
                            .SetRadioListenStateButton.Text = 'Enable Radio'
                    else
                        RadioEnabled = true
                        UI.RadioInterface['RowA-RadioInfo'].RadioInfoState
                            .RadioStatus.TextColor3 =
                        ...
                        UI.RadioInterface['RowA-RadioInfo']
                            .SetRadioListenStateButton.Text = 'Disable Radio'
                    end

                    CommCommanderService:WaitForChild('_CommCommanderServer')
                        :FireServer('SetRadioEnabledState', {
                            ["radioUser"] = LocalPlayer.UserId,
        ...
        UI.RadioInterface['RowA-RadioInfo'].SetRadioListenStateButton
        ...
        UI.RadioInterface['RowC-RadioConfig'].SetRadioBindStateButton
            .MouseButton1Down:Connect(function()
            if not (UI.RadioInterface['RowC-RadioConfig'].SetRadioBindTextBox
                .Text == '') then
                Keybind = UI.RadioInterface['RowC-RadioConfig']
                              .SetRadioBindTextBox.Text
            else
                Keybind = settings.config.default_keybind
            end

            UI.RadioInterface['RowC-RadioConfig'].SetRadioBindTextBox.Text =
                Keybind
            MessageUI.sysMsg(string.format(
                                 'You have changed your radio keybind to: "%s"',
                                 Keybind))
        end)

        UI.RadioInterface['RowC-RadioConfig'].SetRadioNameStateButton
            .MouseButton1Down:Connect(function()
            if not (UI.RadioInterface['RowC-RadioConfig'].SetRadioNameTextBox
                .Text == '') then
                radio_name = UI.RadioInterface['RowC-RadioConfig']
                                 .SetRadioNameTextBox.Text
            else
                radio_name = LocalPlayer.DisplayName
            end

            UI.RadioInterface['RowC-RadioConfig'].SetRadioNameTextBox.Text =
                radio_name
            MessageUI.sysMsg(string.format(
                                 'You have changed your radio display name to: "%s"',
                                 radio_name))
            CommCommanderService:WaitForChild('_CommCommanderServer')
                :FireServer('SetRadioDisplayName', {['radioName'] = radio_name})
        end)

        UI.SetChannelInterface.SetRadioChannelStateButton.MouseButton1Down:Connect(
            function()
                ...
                MessageUI.sysMsg(string.format(
                                     'You have switched your radio channel to: "%s"',
                                     channel))
                CommCommanderService:WaitForChild('_CommCommanderServer')
        ...
        print(string.format(
                  'CommCommander Radio Client has finished loading on player: %s (Took %s seconds)',
                  LocalPlayer.Name, string.sub(tick() - Tick, 1, 5)))
    end
}
