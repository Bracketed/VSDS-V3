return {
    run = function(script, ...)
		...
		function isWhitelisted(plr)
			...
			if not DoorSettings.WHITELISTENABLED then
				return true
			end
		...
		Door2.Bar.ClickHolder.Touched:Connect(function(Tool)
			...
			local plr = MainTool:FindFirstAncestorWhichIsA("Player") or game:GetService("Players"):GetPlayerFromCharacter(MainTool.Parent)
