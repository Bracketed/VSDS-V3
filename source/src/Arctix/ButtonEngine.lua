return {
    run = function(script, ...)
        ...
        function Button.ChangeLED(Color)
            for i, LED in pairs(script.Parent.Parent.Parts:GetDescendants()) do
                if LED.Name == 'ButtonLED' then
                    ...
                    if not Color then
                        TS:Create(LED, TSInfo, {
                            Color = settings['Button-Colors']["Idle-Color"]
                        }):Play()
                    end
                end
            end
        ...
        function Button.ButtonPress()
            local TweenIn = TS:Create(script.Parent.Parent.Parts.ButtonBody
                                          .Button.Button, TSInfo2, {
                CFrame = script.Parent.Parent.Parts.ButtonBody.Button.ButtonIn
                    .CFrame
            })
            local TweenOut = TS:Create(script.Parent.Parent.Parts.ButtonBody
                                           .Button.Button, TSInfo2, {
                CFrame = script.Parent.Parent.Parts.ButtonBody.Button.ButtonOut
                    .CFrame
            })
        ...
        function Button.checkForWhitelist(player)
            if not (settings['Button-Whitelist'].WhitelistEnabled) then
                return true
            end
            for GroupID, GroupRanks in pairs(
                                           settings['Button-Whitelist']
            ...
            for _, UserId in pairs(settings['Button-Whitelist'].WhitelistTable
        ...
        if (settings.ButtonType == 'ProximityPrompt') then
            local ProxAttachment = Instance.new('Attachment', script.Parent
                                                    .Parent.Parts.ButtonSensor)
            ...
            if (settings.ToggleButton) then
                ButtonInteraction.ActionText = 'Toggle Button'
            else
                ButtonInteraction.ActionText = 'Press Button'
            end
            ButtonInteraction.ObjectText =
                string.format('[%s]', string.upper(script.Parent.Parent.Name))

            ButtonInteraction.Triggered:Connect(function(plr)
                Button.ButtonPressed(plr)
            end)
        elseif (settings.ButtonType == 'ClickDetector') then
            ButtonInteraction = Instance.new('ClickDetector', script.Parent
                                                 .Parent.Parts.ButtonSensor)
            ButtonInteraction.MaxActivationDistance = 10
