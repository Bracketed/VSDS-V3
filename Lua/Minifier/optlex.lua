...
local function checkpair(i, j)
    ...
    if t1 == "TK_STRING" or t1 == "TK_LSTRING" or t2 == "TK_STRING" or t2 ==
        "TK_LSTRING" then
        return ""
...
local function do_number(i)
    ...
    elseif not match(z, "[eE]") then -- float
        ...
        if tonumber(q) == 0 and p == "0" then
            ...
            if zeros_cnt > 0 then q = sub(q, 1, #q - zeros_cnt) end
    ...
    if y and y ~= sinfos[i] then
        if opt_details then
            print(
                "<number> (line " .. stoklns[i] .. ") " .. sinfos[i] .. " -> " ..
                    y)
...
local function do_string(I)
    ...
    if z ~= sinfos[I] then
        if opt_details then
            print(
                "<string> (line " .. stoklns[I] .. ") " .. sinfos[I] .. " -> " ..
                    z)
            opt_details = opt_details + 1
        end
        sinfos[I] = z
...
local function do_lstring(I)
    ...
    while true do
        ...
        if ln ~= "" then
            ...
            if match(ln, "%s+$") then
                M.warn.LSTRING =
                    "trailing whitespace in long string near line " ..
                        stoklns[I]
            end
            y = y .. ln
        end
