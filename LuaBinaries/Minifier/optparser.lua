...
local ALPHANUM =
    "etaoinshrdlucmfwypvbgkqjxz_0123456789ETAOINSHRDLUCMFWYPVBGKQJXZ"
...
self _ENV]]):gmatch("%S+") do SKIP_NAME[v] = true end
...
local function preprocess(infotable)
    ...
    for i = 1, #infotable do -- enumerate info table
        ...
        if not uniqtable[name] then -- not found, start an entry
            uniqtable[name] = {decl = 0, token = 0, size = 0}
        end
...
local function stats_summary(globaluniq, localuniq, afteruniq, option) -- luacheck: ignore 431
    ...
    if opt_details then
        ...
        sort(sorted, function(v1, v2) return v1.size > v2.size end)
        ...
        do
            ...
            for i = 1, #sorted do
                local uniq = sorted[i]
                print(fmt(tabf2, uniq.token, uniq.size,
                          avg(uniq.token, uniq.size), uniq.name))
            end
            print(hl)
        ...
        do
            local tabf1, tabf2 = "%8s%8s%8s%10s%8s%10s  %s",
                                 "%8d%8d%8d%10.2f%8d%10.2f  %s"
            local hl = rep("-", 70)
            print(
                "*** local variable list (sorted by allocation order) ***\n" ..
                    hl)
            print(fmt(tabf1, "Decl.", "Token", "Input", "Input", "Output",
                      "Output", "Global"))
            print(fmt(tabf1, "Count", "Count", "Bytes", "Average", "Bytes",
                      "Average", "Name"))
            print(hl)
            for i = 1, #varlist do -- iterate according to order assigned
                ...
                print(fmt(tabf2, uniq.decl, uniq.token, old_s,
                          avg(old_t, old_s), uniq.size,
                          avg(uniq.token, uniq.size), name))
            end
            print(hl)
    ...
    do
        ...
        print(fmt(tabf1, "Variable", "Unique", "Decl.", "Token", "Size",
                  "Average"))
        print(fmt(tabf1, "Types", "Names", "Count", "Count", "Bytes", "Bytes"))
        print(hl)
        print(fmt(tabf2, "Global", uniq_g, decl_g, token_g, size_g,
                  avg(token_g, size_g)))
        print(hl)
        print(fmt(tabf2, "Local (in)", uniq_li, decl_li, token_li, size_li,
                  avg(token_li, size_li)))
        print(fmt(tabf2, "TOTAL (in)", uniq_ti, decl_ti, token_ti, size_ti,
                  avg(token_ti, size_ti)))
        print(hl)
        print(fmt(tabf2, "Local (out)", uniq_lo, decl_lo, token_lo, size_lo,
                  avg(token_lo, size_lo)))
        print(fmt(tabf2, "TOTAL (out)", uniq_to, decl_to, token_to, size_to,
                  avg(token_to, size_to)))
        print(hl .. "\n")
    end
end
...
local function optimize_func1()
    ...
    local function is_strcall(j) -- find f("string") pattern
        ...
        if t1 == "(" and t2 == "<string>" and t3 == ")" then return true end
    end
