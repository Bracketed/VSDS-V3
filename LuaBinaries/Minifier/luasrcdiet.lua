...
local function dump_parser(srcfl)
    ...
    for i = 1, #localinfo do
        ...
        if obj.is_special then msg = msg .. " is_special" end
        msg = msg .. " -> "
        local xref = obj.xref
        for j = 1, #xref do msg = msg .. xref[j] .. " " end
        print(msg)
...
local function process_file(srcfl, destfl)
    ...
    toklist, seminfolist, toklnlist = optlex.optimize(option, toklist,
                                                      seminfolist, toklnlist)
    ...
    print(fmt(tabf1, "Lexical", "Input", "Input", "Input", "Output", "Output",
              "Output"))
    print(fmt(tabf1, "Elements", "Count", "Bytes", "Average", "Count", "Bytes",
              "Average"))
    print(hl)
    for i = 1, #TTYPES do
        local ttype = TTYPES[i]
    ...
    elseif warn.MIXEDEOL then
        print("* WARNING: " ..
                  "output still contains some CRLF or LFCR line endings")
...
local function main()
    local fspec = {}
    local argn, i = #arg, 1
    if argn == 0 then option.HELP = true end
