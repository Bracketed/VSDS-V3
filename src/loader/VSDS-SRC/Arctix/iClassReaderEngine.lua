return {
    run = function(script, ...)
        ...
        local TSInfo = TweenInfo.new(2, Enum.EasingStyle.Linear,
                                     Enum.EasingDirection.InOut)

        function Reader.ChangeLED(Color, Material)
            for i, LED in pairs(
                              script.Parent.Parent.ReaderComponents:GetDescendants()) do
        ...
        function Reader.Flash(Color, AmmountOfTimes)
            for i = 1, AmmountOfTimes do
                Reader.ChangeLED(Color3.fromRGB(143, 142, 144),
                                 Enum.Material.Glass)
                wait(0.2)
        ...
        function Reader.checkForWhitelist(player)
            if not (settings['Reader-Whitelist'].WhitelistEnabled) then
                return true
            end
            for GroupID, GroupRanks in pairs(
                                           settings['Reader-Whitelist']
            ...
            for _, UserId in pairs(settings['Reader-Whitelist'].WhitelistTable
        ...
        function Reader.CardScanned(cardHit)
            ...
            if not table.find(settings.KeyLevel,
                ...
                Reader.Flash(settings['Reader-Colors']["Denied-Color"], 7)
                settings["Reader-Functions"]["Reader-Denied"]()
                return
            end
            ...
            if not Reader.checkForWhitelist(
            ...
            for i, LED in pairs(
                              script.Parent.Parent.ReaderComponents:GetDescendants()) do
                if LED.Name == 'ReaderLED' then
                    local T = TS:Create(LED, TSInfo, {
                        Color = settings['Reader-Colors']['Idle-Color']
                    })
                    T:Play()
                    T.Completed:Wait()
                end
        ...
        for i, Sensor in pairs(
                             script.Parent.Parent.ReaderComponents.ReaderParts:GetDescendants()) do
