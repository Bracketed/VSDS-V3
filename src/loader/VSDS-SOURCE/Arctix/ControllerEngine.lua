return {
    run = function(script, ...)
        ...
        local System = pcall(function()
            if script.Parent.Parent:FindFirstChild('VirtuaAPI') then
                return script.Parent.Parent:WaitForChild('VirtuaAPI')
            else
                return nil
            end
        end)
        ...
        for ButtonName, ButtonFunction in
            pairs(SettingsModule.ControllerButtons) do
            if (typeof(ButtonFunction) == 'function') then
                local Button = Instance.new('TextButton',
                                            SurfaceGUI.Unlocked.Frame)
        ...
        function FunctionTable.UnlockController()
            ...
            if SettingsModule.ControllerTweens then
                game:GetService('TweenService'):Create(LED, TweenInfo.new(5), {
                    Color = SettingsModule.ControllerUnlockedLEDColor
                }):Play()
            else
        ...
        function FunctionTable.LockController()
            ...
            if SettingsModule.ControllerTweens then
                game:GetService('TweenService'):Create(LED, TweenInfo.new(5), {
                    Color = SettingsModule.ControllerLockedLEDColor
                }):Play()
            else
                LED.Color = SettingsModule.ControllerLockedLEDColor
            end
        ...
        function FunctionTable.checkForWhitelist(player)
            if not (SettingsModule.Whitelist.WhitelistEnabled) then
                return true
            end
            for GroupID, GroupRanks in pairs(
                                           SettingsModule.Whitelist
                                               .WhitelistTable.WhitelistedGroups) do
                local gr = player:GetRankInGroup(GroupID)
                for _, RankID in pairs(GroupRanks) do
                    if (gr == RankID) then return true end
                end
            end
            for _, UserId in pairs(SettingsModule.Whitelist.WhitelistTable
        ...
        function FunctionTable.CardScanned(cardHit)
            if not cardHit.Parent:FindFirstChild("VirtuaCard") then
                return
            end
            if not cardHit.Parent:FindFirstChild("CardRank") then
                return
            end
            if not table.find(SettingsModule.KeyLevel,
                              cardHit.Parent:FindFirstChild("CardRank").Value) then
                return
            end
            if not game:GetService('Players'):FindFirstChild(cardHit.Parent
                                                                 .Parent.Name) then
                return
            end
            if not FunctionTable.checkForWhitelist(
                game:GetService('Players')[cardHit.Parent.Parent.Name]) then
                return
            end
        ...
        function FunctionTable.isButtonFunction(FunctionName)
            for ButtonName, _ in pairs(SettingsModule.ControllerButtons) do
                if ButtonName == FunctionName then return true end
            end
        ...
        function FunctionTable.getButtonFunction(FunctionName)
            for ButtonName, ButtonFunction in pairs(
                                                  SettingsModule.ControllerButtons) do
                if ButtonName == FunctionName then
                    return {Name = ButtonName, Function = ButtonFunction}
                end
            end
        ...
        if SettingsModule.ControllerTweens then
            Debounce.Changed:Connect(function()
                ...
                task.wait(5)
                game:GetService('TweenService'):Create(LED, TweenInfo.new(2), {
                    Color = SettingsModule.ControllerDisabledLEDColor
                }):Play()
                task.wait(SettingsModule.ControllerDebounce - 7)
                game:GetService('TweenService'):Create(LED, TweenInfo.new(2), {
                    Color = ColourToGoBackTo
                }):Play()
            end)
        end
        ...
        for _, Button in pairs(SurfaceGUI.Unlocked.Frame:GetChildren()) do
            if Button:IsA('TextButton') then
                Button.MouseButton1Down:Connect(function()
                    local IsButtonFunction =
                        FunctionTable.isButtonFunction(Button.Name)
                    local Function =
                        FunctionTable.getButtonFunction(Button.Name)
                    if (typeof(Function.Function) == 'nil') then
                        return
                    end
