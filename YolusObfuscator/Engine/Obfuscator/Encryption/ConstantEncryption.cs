...
namespace YolusCORE.Obfuscator.Encryption
{
	...
	public class ConstantEncryption
	{
		...
		public Decryptor GenerateGenericDecryptor(MatchCollection matches)
		{
			...
			return new Decryptor("TBRACKETED_STR_DEC_GENERIC", len);
		}
		...
		public string EncryptStrings()
		{
			...
			else
			{
				...
				foreach (Match m in matches)
				{
					...
					captured = captured.Substring(13);
					Decryptor dec = new Decryptor("TBRACKETED_STR_ENCRYPT" + n++, m.Length);
			...
			if (_settings.EncryptImportantStrings)
			{
				...
				foreach (Match m in matches)
				{
					...
					Decryptor dec = new Decryptor("TBRACKETED_STR_ENCRYPT_IMPORTANT" + n++, m.Length);
